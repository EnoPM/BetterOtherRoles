        ...
        private static class SendChatPatch {
            static bool Prefix(ChatController __instance) {
                ...
                if (AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started) {
                    ...
                    {
                        var playerName = text.Substring("/shield ".Length);
                        var target = CachedPlayer.AllPlayers.FirstOrDefault(x =>
                            x.Data.PlayerName.ToLowerInvariant() == playerName.ToLowerInvariant());
                        if (target != null)
                        {
